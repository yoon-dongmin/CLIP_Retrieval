...
def gen_neg_samples(verbs,objs,commands,image_embeddings,vo_dict,n=1):
    ret_set = []
    for i in range(len(image_embeddings)):
        verb, obj, command, image_embedding = verbs[i], objs[i], commands[i], image_embeddings[i]
        l = [command]
        ...
        all_o = [obj]
        ret_embedds = torch.zeros((len(image_embedding),n), dtype=torch.float16).cuda()
        ret_embedds[:,0]= image_embedding  
        idx = 1
        while len(all_o) < n:
            num = random.randrange(len(objs))
            ...
            if (objs[num] not in vo_dict[verb]) and (objs[num] not in all_o):
                ...
                ret_embedds[:,idx]= image_embeddings[num]                
        ...
        if len(all_o) == n:
            ...
            ret_set.append(l) #[[command,embeddings]]
            print(len(ret_set))
