...
from data_utils2 import CustomDataset, CustomDataset2
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_acc, model):
        ...
        torch.save(model.state_dict(), 'models2/checkpoint.pt')
        # torch.save({
        # 'epoch': epoch,
...
def eval(clip_model, testloader, eval_loss, ret_acc1, ret_acc2,bsz,num_comm=5):
    ...
    for idx, (sentence, ret_objs, val) in enumerate(tqdm(testloader)):
        ...
        ground_truth = torch.zeros((bsz), dtype=torch.float16).cuda()
...
def clip_finetune(num_epochs: int, clip_model_name: str, learning_rate: float, batch_size: int
    ...
    # training_path: Path = Path(f"trained_models/clip_finetuned_on_fiq_{clip_model_name}_{training_start}")
    ...
    ###Data loading###
    with open('data/label_embedding.json') as f: #vo_dict.json is generated by gen_data.py
        for line in f:
    ...
    testloader = torch.utils.data.DataLoader(test_data, batch_size=512, drop_last=True)
    # print(len(trainloader)) #141360
    # print(len(testloader)) #17670
    ...
    for epoch in range(num_epochs): #tqdm module을 이용하면 iteration 진행 상황을 progress bar로 나타낼 수 있다
        sum_loss = 0.0
        print('EPOCH', epoch + 1)
        for idx, (image_embedding,command,val) in enumerate(tqdm(trainloader)):
            # print(image_embedding)ll
            # print(command)
            # print(val)
            optimizer.zero_grad()
            image_features = image_embedding.to('cuda').to(torch.float16)
    ...
    if plot_fig:
        ...
        utils.acc_plot(plot_acc, 'Top1 & Top2 Accuracy',
                'results2/accuracy.png')



if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument("--num-epochs", default=200, type=int, help="number training epochs")
