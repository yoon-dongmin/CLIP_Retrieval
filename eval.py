...
def verb_eval(clip_model, testloader,bsz,num_comm=5):
    top1, top2, n = 0., 0., 0.
    for idx, (sentence, ret_objs,_) in enumerate(tqdm(testloader)):
...
def test(clip_model_name: str, batch_size: int):
    ...
    ###Data loading###
    with open('data/RN101/test1_embedding.json') as f: #vo_dict.json is generated by gen_data.py
        for line in f:
    ...
    alpha = 0.5
    model1, preprocess = clip.load("RN101")
    clip_model, preprocess = clip.load("RN101")
    saved_state_dict = torch.load("RN101_results(n50)/best.pt")
    ...
    testloader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, drop_last=True)
    ...
    ###model evaluation###
    ###1.verb###
    verb_eval(clip_model, testloader,bsz=batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--clip-model-name", default="RN101", type=str, help="CLIP model to use, e.g 'RN101', 'RN101x4'")
